...
- The runtime type introspection required for DRY is modeled after [Zod](https://github.com/colinhacks/zod/).  
  Zod was even used for an early prototype and I plan to use it for future projects. However, for this use case, a lightweight, tailored alternative to Zod seemed to be able to reduce code runtime, complexity, and reliance on internal state. If Zod's `transform` would have made it easy to transform entire data structures, not just individual values, it would have been the prime candidate; otherwise, the use of Zod would—for this application—just require duplication of work at multiple levels.

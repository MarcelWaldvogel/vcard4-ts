  ...
  for (const propertyName of Object.keys(vcard)) {
    ...
    if (isKnownProperty(propertyName)) {
      ...
      if (Array.isArray(propertyInfo)) {
        ...
        grouped[groupName] ??= {};
        (grouped[groupName][propertyName] as
          | SingleVCardProperty<any>
          | undefined) = propertyInfo;
      }
    }
  }
  if ('x' in vcard) {
    for (const [propertyName, propertyInfo] of Object.entries(vcard.x!)) {
      for (const e of propertyInfo) {
        ...
        grouped[groupName] ??= {};
        grouped[groupName].x ??= {};
        const x = grouped[groupName].x!;
        if (propertyName in x) {
          (x[propertyName] as SingleVCardProperty<any>[]).push(e);
        } else {
          (x[propertyName] as SingleVCardProperty<any>[]) = [e];
        }
      }
    }
  }
